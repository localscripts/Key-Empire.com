...
export class FortniteAffiliateService {
  ...
  static getInstance(): FortniteAffiliateService {
    if (!FortniteAffiliateService.instance) {
      FortniteAffiliateService.instance = new FortniteAffiliateService()
    }
    return FortniteAffiliateService.instance
  }
  ...
  private async loadData(): Promise<LocalAffiliateData> {
    try {
      ...
      const data = await fs.readFile(FORTNITE_DATA_FILE, "utf-8")
  ...
  private async saveData(data: LocalAffiliateData): Promise<void> {
    try {
      await this.ensureDataDirectory()
      await fs.writeFile(FORTNITE_DATA_FILE, JSON.stringify(data, null, 2))
    } catch (error) {}
  }
  ...
  private async scrapeFortniteSite(
    site: any,
    ...
    try {
      const response = await fetch(site.url, {
        headers: {
          "User-Agent": "KeyEmpire-Fortnite-Scraper/1.0",
  ...
  async getFortniteResellersData(affiliateCode = "voxlisnet"): Promise<{ [productName: string]: ProductResellers }> {
    try {
      ...
      if (timeDiff > CACHE_DURATION || Object.keys(data.resellers).length === 0) {
        ...
        const scrapePromises = FORTNITE_RESELLER_SITES.map(async (site) => {
          try {
            const result = await this.scrapeFortniteSite(site, affiliateCode)
            return result
          } catch (error) {
            return {}
          }
