...
export class LocalAffiliateService {
        ...
        for (const [productName, details] of Object.entries(offers as any)) {
          ...
          for (const [durationKey, pricing] of Object.entries(details as any)) {
            if (pricing && typeof pricing === "object" && pricing.url && pricing.price) {
              const processedUrl = this.processAffiliateUrl(pricing.url, affiliateCode)
  ...
  private processAffiliateUrl(url: string, affiliateCode: string): string {
    if (url.includes("robloxcheatz.com")) {
      // Extract the base domain and convert to affiliate format
      const urlObj = new URL(url)
      const newUrl = `${urlObj.protocol}//${urlObj.hostname}/affiliate/${affiliateCode}`
      return newUrl
    }

    // Replace all instances of voxlisnet with the affiliate code
    const processedUrl = url.replace(/\/affiliate\/voxlisnet/g, `/affiliate/${affiliateCode}`)

    // Also handle cases where the URL might have other affiliate codes that need to be replaced
    const finalUrl = processedUrl.replace(/\/affiliate\/[^/?]+/g, `/affiliate/${affiliateCode}`)
          ...
          for (const [resellerKey, reseller] of Object.entries(resellers)) {
            ...
            for (const [durationKey, duration] of Object.entries(reseller.durations)) {
              processedDurations[durationKey] = {
                ...duration,
                url: this.processAffiliateUrl(duration.url, affiliateCode),
              }
            }
  ...
  async getProductResellers(productName: string, affiliateCode = "voxlisnet"): Promise<ProductResellers> {
    try {
