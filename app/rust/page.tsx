      ...
      const individualPromises = potentialProducts.map(async (product) => {
        if (product.title === "Cryptic") {
          ...
          try {
            ...
            const response = await fetch(`/api/rust/resellers/${product.title.toLowerCase()}?affiliate=${affiliateCode}`, {
              signal: controller.signal,
              headers: { "Cache-Control": "no-cache" },
            })
    ...
    async (productTitle: string, platform?: string) => {
      ...
      try {
        ...
        if (productTitle === "Cryptic" && platform) {
          ...
          apiUrl = `/api/rust/resellers/${productTitle.toLowerCase()}?affiliate=${affiliateCode}`
        }
