...
import { RUST_PRODUCTS_LIST, parsePrice } from "../../lib/rust-products-data"
...
const SelectionsPage = () => {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (hash) {
        ...
        const matchingProduct = RUST_PRODUCTS_LIST.find(
  ...
  useEffect(() => {
    const fetchAllProductData = async () => {
      ...
      try {
        ...
        const batchResponse = await fetch(`/api/rust/resellers?affiliate=${affiliateCode}`, {
          method: "GET",
          headers: {
        ...
        if (batchResponse.ok) {
          ...
          Object.entries(batchData).forEach(([productKey, productData]: [string, any]) => {
            // Find matching product in RUST_PRODUCTS_LIST
            const matchingProduct = RUST_PRODUCTS_LIST.find((p) => p.title.toLowerCase() === productKey.toLowerCase())
      ...
      const potentialProducts = RUST_PRODUCTS_LIST.filter(
        (product) =>
          !product.hidden &&
          [
            "wave",
        ...
        if (product.title === "Cryptic") {
          ...
          const crypticPromises = crypticPlatforms.map(async (platform) => {
            try {
              ...
              const response = await fetch(`/api/rust/products/cryptic-${platform}?affiliate=${affiliateCode}`, {
                signal: controller.signal,
          ...
          try {
            ...
            const response = await fetch(`/api/rust/products/${product.title.toLowerCase()}?affiliate=${affiliateCode}`, {
              signal: controller.signal,
              headers: { "Cache-Control": "no-cache" },
            })
  ...
  useEffect(() => {
    const silentFetch = async () => {
      try {
        await fetch("/api/rust/resellers", { method: "GET" })
  ...
  const visibleProducts = useMemo(() => {
    const allVisibleProducts = RUST_PRODUCTS_LIST.filter((product) => !product.hidden)
    ...
    async (productTitle: string, platform?: string) => {
      ...
      try {
        ...
        if (productTitle === "Cryptic" && platform) {
          apiUrl = `/api/rust/products/cryptic-${platform}?affiliate=${affiliateCode}`
        } else {
          apiUrl = `/api/rust/products/${productTitle.toLowerCase()}?affiliate=${affiliateCode}`
        }
  ...
  const selectedProductDetails = selectedProduct ? RUST_PRODUCTS_LIST.find((s) => s.title === selectedProduct) : null
