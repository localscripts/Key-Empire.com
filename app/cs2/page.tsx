...
export default function CS2Page() {
  ...
  const [filteredSelections, setFilteredSelections] = useState(CS2_PRODUCTS_LIST.filter((product) => !product.hidden))
  ...
  useEffect(() => {
    const fetchAllProductData = async () => {
      ...
      try {
        const batchResponse = await fetch(`/api/cs2/resellers?affiliate=${affiliate}`, {
          method: "GET",
          headers: {
      ...
      const individualPromises = CS2_PRODUCTS_LIST.map(async (product) => {
        try {
          const response = await fetch(`/api/cs2/products/${product.title.toLowerCase()}?affiliate=${affiliate}`, {
            signal: controller.signal,
  ...
  const fetchProductResellers = useCallback(
    async (productTitle: string) => {
      ...
      try {
        const apiUrl = `/api/cs2/products/${productTitle.toLowerCase()}?affiliate=${affiliate}`
