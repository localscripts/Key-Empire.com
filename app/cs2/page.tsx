...
const SelectionsPage = () => {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (hash) {
        ...
        const matchingProduct = CS2_PRODUCTS_LIST.find(
  ...
  useEffect(() => {
    const fetchAllProductData = async () => {
      ...
      try {
        ...
        const batchResponse = await fetch(`/api/cs2/resellers?affiliate=${affiliateCode}`, {
          method: "GET",
          headers: {
        ...
        if (batchResponse.ok) {
          ...
          Object.entries(batchData).forEach(([productKey, productData]: [string, any]) => {
            // Find matching product in CS2_PRODUCTS_LIST
            const matchingProduct = CS2_PRODUCTS_LIST.find((p) => p.title.toLowerCase() === productKey.toLowerCase())
      ...
      const potentialProducts = CS2_PRODUCTS_LIST.filter(
        (product) =>
          !product.hidden &&
          [
            "wave",
        ...
        if (product.title === "Cryptic") {
          ...
          const crypticPromises = crypticPlatforms.map(async (platform) => {
            try {
              ...
              const response = await fetch(`/api/cs2/products/cryptic-${platform}?affiliate=${affiliateCode}`, {
                signal: controller.signal,
          ...
          try {
            ...
            const response = await fetch(`/api/cs2/products/${product.title.toLowerCase()}?affiliate=${affiliateCode}`, {
              signal: controller.signal,
              headers: { "Cache-Control": "no-cache" },
            })
  ...
  useEffect(() => {
    const silentFetch = async () => {
      try {
        await fetch("/api/cs2/resellers", { method: "GET" })
  ...
  const visibleProducts = useMemo(() => {
    const allVisibleProducts = CS2_PRODUCTS_LIST.filter((product) => !product.hidden)
    ...
    async (productTitle: string, platform?: string) => {
      ...
      try {
        ...
        if (productTitle === "Cryptic" && platform) {
          apiUrl = `/api/cs2/products/cryptic-${platform}?affiliate=${affiliateCode}`
        } else {
          apiUrl = `/api/cs2/products/${productTitle.toLowerCase()}?affiliate=${affiliateCode}`
        }
  ...
  const selectedProductDetails = selectedProduct ? CS2_PRODUCTS_LIST.find((s) => s.title === selectedProduct) : null
