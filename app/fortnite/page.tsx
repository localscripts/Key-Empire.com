...
export default function FortnitePage() {
  ...
  const [filteredSelections, setFilteredSelections] = useState(
    FORTNITE_PRODUCTS_LIST.filter((product) => !product.hidden),
  )
  ...
  useEffect(() => {
    const fetchAllProductData = async () => {
      ...
      try {
        const batchResponse = await fetch(`/api/fortnite/resellers?affiliate=${affiliate}`, {
          method: "GET",
          headers: {
      ...
      const individualPromises = FORTNITE_PRODUCTS_LIST.map(async (product) => {
        try {
          const response = await fetch(`/api/fortnite/products/${product.title.toLowerCase()}?affiliate=${affiliate}`, {
            signal: controller.signal,
  ...
  const fetchProductResellers = useCallback(
    async (productTitle: string) => {
      ...
      try {
        const apiUrl = `/api/fortnite/products/${productTitle.toLowerCase()}?affiliate=${affiliate}`
