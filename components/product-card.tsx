...
export default function ProductCard({
  ...
  return (
    <button
      ...
      style={{
        opacity: 0,
        animation: `fastFadeInUp 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94) ${index * 0.05}s forwards`,
      }}
    >
      <div
        className={`relative bg-white/95 dark:bg-gray-800/95 backdrop-blur-lg rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 group-hover:scale-[1.03] group-hover:-translate-y-1 border-2 ${getBorderColor(shadeColor)}`}
      >
        {/* Image Container */}
        <div className="relative aspect-[4/3] overflow-hidden">
          ...
          <div className="absolute inset-0 opacity-20">
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white dark:via-gray-300 to-transparent transform -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-700 ease-out"></div>
          </div>
          ...
          <div className="absolute inset-0 p-2 sm:p-3 flex items-center justify-center">
            <div className="relative w-full h-full">
              <Image
                ...
                alt={title}
                fill
                className="object-contain transform group-hover:scale-110 transition-transform duration-300 drop-shadow-2xl"
                draggable={false}
              />
            </div>
          </div>
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-end justify-center pb-6"></div>
        </div>
        ...
        <div className="p-2 sm:p-3 space-y-2">
          ...
          <div className="space-y-1 sm:space-y-2">
            <h3 className="text-sm sm:text-base font-bold text-gray-900 dark:text-gray-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-200 leading-tight">
              {title}
            </h3>
          </div>
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="space-y-0.5 sm:space-y-1 flex-shrink-0">
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">
                {resellers.includes("sellers") ? resellers : `${resellers} sellers`}
              </div>
            </div>
            ...
            <button
              className={`${
                isSelected
                  ...
                  : "bg-blue-600 hover:bg-blue-700 cursor-pointer hover:scale-110 active:scale-95"
              } text-white rounded-lg font-semibold text-xs transition-all duration-200 shadow-md hover:shadow-lg transform border border-white/20 hover:border-white/40 backdrop-blur-sm relative overflow-hidden group/btn px-2.5 py-1.5 sm:px-3 sm:py-2 flex-shrink-0 min-w-0`}
              onClick={(e) => {
                e.stopPropagation()
                handleProductClick()
              }}
            >
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent -skew-x-12 -translate-x-full group-hover/btn:translate-x-full transition-transform duration-500"></div>
              ...
              <span className="relative z-10 flex items-center gap-1 whitespace-nowrap">
                {isSelected ? (
                  <>
                    ...
                    <span className="hidden sm:inline text-xs">Selected</span>
                    <span className="sm:hidden text-xs">✓</span>
                  </>
                ) : (
                  <>
                    <svg
                      className="w-3 h-3 flex-shrink-0 group-hover/btn:rotate-12 transition-transform duration-300"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
              ...
              {!isSelected && (
                <div className={`absolute inset-0 rounded-lg border border-blue-400 animate-pulse opacity-40`}></div>
              )}
            </button>
          </div>
        </div>

        <div className="absolute inset-0 rounded-xl border-2 border-transparent group-hover:border-gradient-to-r group-hover:from-blue-400 group-hover:to-purple-400 transition-all duration-200 pointer-events-none"></div>
      </div>
