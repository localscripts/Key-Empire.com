...
export default function GetStartedModal({ isOpen, onClose }: GetStartedModalProps) {
  ...
  return (
    <>
      {/* Modal Backdrop */}
      <div
        className="fixed inset-0 bg-black/80 z-[130] backdrop-blur-sm"
        style={{
      ...
      <div
        ...
        className="fixed left-1/2 top-1/2 w-[95%] max-w-lg bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl z-[140] overflow-hidden select-none border border-gray-700"
        style={{
          opacity: isClosing ? 0 : 1,
          transform: isClosing
      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-2xl font-bold text-gray-100">Choose Your Game</h2>

            {/* Close Button */}
            <button
              onClick={handleCloseModal}
              className="bg-gray-800 hover:bg-gray-700 text-gray-400 rounded-lg p-2 transition-all duration-200"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
        ...
        >
          {allGames.map((game, index) => (
            <div
              key={index}
              className="flex items-center justify-between p-4 mb-3 rounded-xl border transition-all duration-200 cursor-pointer group relative overflow-hidden border-gray-700 hover:border-green-600 hover:bg-green-900/20 bg-gray-800 shadow-sm hover:shadow-md"
              style={{
                opacity: isClosing ? 0 : 0,
            ...
            >
              ...
              <div className="flex items-center gap-4 flex-1">
                {/* Icon */}
                <div className="w-12 h-12 flex items-center justify-center rounded-xl transition-all duration-200 overflow-hidden relative bg-gray-700 group-hover:bg-green-900/30">
                  <Image
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-lg transition-colors text-gray-100 group-hover:text-green-300">
                    {game.name}
                  </h3>
                </div>
              </div>
