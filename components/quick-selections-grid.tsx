...
import Link from "next/link" // Keep Link for other items
...
const quickLinks: QuickLink[] = [
  ...
  },
  {
    ...
    href: "/selections", // This href will be ignored for the button
...
export default function QuickSelectionsGrid({ onOpenGetStartedModal }: QuickSelectionsGridProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {quickLinks.map((link, index) => {
        const isGetStarted = link.title === "Get Started"
        const commonProps = {
          className:
            "group block p-8 bg-white/95 dark:bg-gray-800/95 backdrop-blur-lg rounded-2xl border border-gray-200 dark:border-gray-700 shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105 relative overflow-hidden",
          style: {
            opacity: 0,
        ...
        const content = (
          <>
            ...
            <h3
              className={`text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2 transition-colors select-none relative z-10 text-left ${
                link.title === "Discord"
            ...
            <p className="text-gray-600 dark:text-gray-400 select-none relative z-10 text-left">{link.description}</p>
          </>
        )
        ...
        if (isGetStarted) {
          return (
            <button
              key={index}
              {...commonProps}
              onClick={onOpenGetStartedModal} // Directly call the modal opener
            >
              {content}
            </button>
          )
          ...
          return (
            <Link href={link.href} key={index} {...commonProps}>
              {content}
            </Link>
          )
        }
