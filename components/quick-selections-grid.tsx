...
const quickLinks: QuickLink[] = [
  ...
  },
  {
    ...
    // no href; opens GetStartedModal
    color: "from-green-500 to-green-600",
  },
  {
    title: "Referrals",
    description: "Find your perfect tool",
    // no href; opens AffiliateModal
        ...
        {quickLinks.map((link, index) => {
          const cardContent = (
            <>
              ...
              <p className="text-gray-600 dark:text-gray-400 select-none relative z-10">
                {link.description}
              </p>
            </>
          )

          // Button behavior for Get Started and Referrals
          if (link.title === "Referrals") {
            return (
              <button
                key={index}
                onClick={() => setIsAffiliateModalOpen(true)}
                className="group block p-8 bg-white/95 dark:bg-gray-800/95 backdrop-blur-lg rounded-2xl border border-gray-200 dark:border-gray-700 shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105 relative overflow-hidden text-left w-full"
                style={{
                  opacity: 0,
